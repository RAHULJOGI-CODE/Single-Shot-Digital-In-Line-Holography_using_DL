...
# Normalizes values to within a 0 to 2π range
def unwrap(x):
    y = x % (2 * np.pi)
    return torch.where(y > np.pi, 2*np.pi - y, y)

# Used for transitioning between spatial and frequency domains.
def fft2dc(x): 
...
#  Resolves discontinuities in the phase caused by wrapping (jumps from π to -π).
# a process used to reconstruct the original phase from a wrapped phase map.
...
# Computes the phase shifts caused by light wave propagation
