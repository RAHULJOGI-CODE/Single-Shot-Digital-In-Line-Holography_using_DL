...
# To measure the reconstruction quality based on how well the reconstructed hologram matches the original.
...
class RECLoss(nn.Module):
    ...
    # Simulates the wavefront propagation.
    ...
    # Rolls the tensor along a specified axis by n positions, useful for shifting operations.
    ...
    # Applies FFT shift to the tensor, centering the low-frequency components.
...
def dwt_init(x):
    ...
    # This process effectively captures both the coarse and fine details of the input image
    
    # downsampling x along the height dimension (splitting every other row) and dividing by 2 for normalization
    x01 = x[:, :, 0::2, :] / 2
    x02 = x[:, :, 1::2, :] / 2
...
def iwt_init(x):
    ...
    # thsi process reconstructs an image from its wavelet-transformed components.
    ...
    # Each sub-band (x1, x2, x3, x4) represents different frequency components of the wavelet-transformed image
